...
internal class `XtreamParserImpl.kt` @Inject constructor(
    ...
    override suspend fun getInfo(input: XtreamInput): `XtreamInfo.kt` {
        val (basicUrl, username, password, _) = input
        val infoUrl = XtreamParser.createInfoUrl(basicUrl, username, password)
        return checkNotNull(utils.newCall<`XtreamInfo.kt`>(infoUrl))
    }
    ...
    override suspend fun getXtreamOutput(input: XtreamInput): XtreamOutput {
        ...
        val info: `XtreamInfo.kt` = utils.newCall(infoUrl) ?: return XtreamOutput()
